...
function fixup_xauthority() {
    while :; do
        sleep 1s
        xauth_file="$(ls -1t /run/user/$(id -u)/snap.thin-client-session/.mutter-Xwaylandauth.* | head -n1)"
...
function link_wayland_socket() {
    private_socket=/run/user/$(id -u)/snap.thin-client-session/wayland-0
...
function link_pulse_socket() {
    private_socket=/run/user/$(id -u)/snap.thin-client-session/pulse
...
function link_pipewire_socket() {
    private_socket=/run/user/$(id -u)/snap.thin-client-session/pipewire-0
...
exec /snap/bin/thin-client-session
